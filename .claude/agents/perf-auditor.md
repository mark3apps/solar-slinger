...
- **Per-substep loops iterate `game.bodies._awake`**, never `game.bodies`. Walking the full array
  10–15× a frame just to skip dormants measured **~1.4 ms, ~40% of sim time** (at the old 8,000-body
  scale; the shape of the win is what matters, not the absolute). The awake list holds
