...
// Restored in the `finally` below — never on the happy path alone; the game's
// randomness is deliberate in PLAY, and this only ever covers the harness.
...
try {
  ...
  // GUARANTEED RESTORE. Math.random is a GLOBAL, and the pin above only
  // makes sense for the duration of this harness — a suite that throws part
  // way through and leaves it patched would silently turn every runtime spawn
  // in the page deterministic, which is the hardest kind of bug to see. The
  // bench runner spawns a fresh process per suite so it cannot reach another
  // measurement today, but that is a property of the DRIVER, not of this file.
